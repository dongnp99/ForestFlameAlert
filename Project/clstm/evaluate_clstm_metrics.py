"""
evaluate_clstm_metrics.py — Đánh giá đầy đủ ConvLSTM
======================================================
...
Dùng file .npy đã precomputed (không cần chạy lại GPU inference):
    models/model_20260428_1418/prob_map_val.npy
    models/model_20260428_1418/targets_val.npy
    models/model_20260428_1418/prob_map_test.npy
    models/model_20260428_1418/targets_test.npy

Chạy từ thư mục này:
    python evaluate_clstm_metrics.py
"""
...
MODEL_DIR = HERE / "models" / "model_20260428_1418"

#  Cấu hình domain metrics ─
P_MIN      = 0.05   # Precision tối thiểu để cảnh báo có thể hành động
K_PCT      = 0.05   # Top 5% diện tích được cảnh báo (deployment budget)
FRES_ALPHA = 0.7    # Trọng số recall trong FRES (0.7 = ưu tiên không bỏ sót)
...
def _best_threshold_f2(y_true: np.ndarray, y_prob: np.ndarray) -> float:
    """Tìm threshold tối ưu theo F2 — chạy trên val set."""
    prec, rec, thresholds = precision_recall_curve(y_true, y_prob)
    beta2 = 4.0
    f2 = (1 + beta2) * prec * rec / (beta2 * prec + rec + 1e-9)
    best_idx = int(np.argmax(f2[:-1]))  # bỏ điểm cuối của sklearn
    return float(thresholds[best_idx])


def _recall_at_pmin(y_true: np.ndarray, y_prob: np.ndarray) -> dict:
    """Recall cao nhất khi Precision >= P_MIN trên PR curve."""
    prec, rec, thresholds = precision_recall_curve(y_true, y_prob)
    valid = prec >= P_MIN
    if not valid.any():
...
def _pei(y_true: np.ndarray, y_prob: np.ndarray) -> dict:
    """Patrol Efficiency Index = Recall@K / K_PCT."""
    n = len(y_true)
    k = max(1, int(n * K_PCT))
...
def _fres(y_true: np.ndarray, y_prob: np.ndarray) -> dict:
    """FRES = FRES_ALPHA * Recall@K + (1 - FRES_ALPHA) * Precision@K."""
    n = len(y_true)
    k = max(1, int(n * K_PCT))
    top_k = np.argsort(y_prob)[::-1][:k]
    total_fires = float(y_true.sum())
...
def compute_metrics(y_true: np.ndarray, y_prob: np.ndarray, threshold: float) -> dict:
    """Tính đầy đủ tất cả metrics với threshold đã cho."""
...
def print_metrics(m: dict):
    logging.info("=" * 60)
    logging.info("TEST METRICS — ConvLSTM")
...
# ================================================================
# SECTION 2 — LOAD .NPY FILES
# ================================================================

logging.info("Loading val set .npy files from: %s", MODEL_DIR)
prob_val    = np.load(MODEL_DIR / "prob_map_val.npy")
targets_val = np.load(MODEL_DIR / "targets_val.npy")
logging.info("Val size: %s | fire_rate=%.5f", f"{len(targets_val):,}", targets_val.mean())

logging.info("Loading test set .npy files...")
prob_test    = np.load(MODEL_DIR / "prob_map_test.npy")
targets_test = np.load(MODEL_DIR / "targets_test.npy")
logging.info("Test size: %s | fire_rate=%.5f", f"{len(targets_test):,}", targets_test.mean())

#  Tìm threshold tối ưu trên val 
best_threshold = _best_threshold_f2(targets_val, prob_val)
logging.info("Best threshold (F2 on val): %.4f", best_threshold)

#  Tính metrics trên test 
metrics = compute_metrics(targets_test, prob_test, best_threshold)
print_metrics(metrics)


# ================================================================
# SECTION 3 — PR CURVE PLOT
# ================================================================

prec_curve, rec_curve, _ = precision_recall_curve(targets_test, prob_test)
fire_rate = targets_test.mean()

fig, ax = plt.subplots(figsize=(7, 5))
ax.plot(rec_curve, prec_curve, color="#E05C2A", lw=2,
        label=f"ConvLSTM (AUC-PR={metrics['auc_pr']:.4f})")
ax.axhline(y=P_MIN, color="gray", ls="--", lw=1,
           label=f"P_min = {P_MIN:.0%}")
ax.axhline(y=fire_rate, color="#2A7DE0", ls=":", lw=1,
...
ax.set_title("PR Curve — ConvLSTM (Test set)")
ax.legend(loc="upper right", fontsize=8)
ax.set_xlim(0, 1)
ax.set_ylim(0, max(prec_curve.max() * 1.1, P_MIN * 2))
plt.tight_layout()

out_fig = MODEL_DIR / "clstm_pr_curve.png"
...
# ================================================================
# SECTION 4 — LƯU KẾT QUẢ
# ================================================================

out_txt = MODEL_DIR / "clstm_full_metrics.txt"
lines = [
    "ConvLSTM — Full Evaluation",
    "=" * 60,
