"""
evaluate_v5_metrics.py — Đánh giá đầy đủ XGBoost v5
=====================================================
...
from xgb_config import DATA_PATH, FEATURE_COLS, TRAIN_END_DATE, VAL_END_DATE
...
def _best_threshold_f2(y_true: np.ndarray, y_prob: np.ndarray) -> float:
    """Tìm threshold tối ưu theo F2 trên tập đang xét (dùng trên val set)."""
    prec, rec, thresholds = precision_recall_curve(y_true, y_prob)
    beta2 = 4.0
    num = (1 + beta2) * prec * rec
    den = beta2 * prec + rec + 1e-9
    f2_scores = num / den
    best_idx = int(np.argmax(f2_scores[:-1]))  # bỏ điểm cuối của sklearn
...
def print_metrics(m: dict, split: str):
    logging.info("=" * 60)
    logging.info("%s METRICS", split.upper())
...
# ================================================================
# SECTION 2 — LOAD DATA & PREDICT
# ================================================================

def load_split(date_filters: list) -> tuple[np.ndarray, np.ndarray]:
    """Load một split từ parquet, trả về (y_true, y_prob)."""
    df = pd.read_parquet(
        DATA_PATH,
...
#  Load model 
MODEL_PATH = HERE / "xgb_human_features_tuned_v5_pathways.json"
...
y_test, prob_test = load_split([("date", ">", pd.Timestamp(VAL_END_DATE))])
...
ax.plot(rec_curve, prec_curve, color="#2A7DE0", lw=2, label=f"XGBoost v5 (AUC-PR={metrics['auc_pr']:.4f})")
ax.axhline(y=P_MIN, color="gray", ls="--", lw=1, label=f"P_min = {P_MIN:.0%}")
ax.axhline(y=fire_rate, color="#E05C2A", ls=":", lw=1, label=f"Baseline (fire rate = {fire_rate:.4%})")
ax.scatter(metrics["recall"], metrics["precision"], color="red", zorder=5,
           label=f"Threshold={metrics['threshold']:.3f}\n(F2={metrics['f2']:.4f})")
ax.set_xlabel("Recall")
ax.set_ylabel("Precision")
ax.set_title("PR Curve — XGBoost v5 (Test set)")
ax.legend(loc="upper right", fontsize=8)
ax.set_xlim(0, 1)
ax.set_ylim(0, max(prec_curve.max() * 1.1, P_MIN * 2))
plt.tight_layout()

out_fig = HERE / "v5_pr_curve.png"
...
out_txt = HERE / "v5_full_metrics.txt"
lines = [
    "XGBoost v5 — Full Evaluation",
    "=" * 60,
