"""
evaluate_rf_metrics.py — Đánh giá đầy đủ Random Forest baseline
================================================================
...
def _find_latest_model() -> tuple[Path, Path]:
    """Tìm cặp (model, imputer) .joblib mới nhất trong results/."""
    candidates = sorted(RESULTS_DIR.glob("random_forest_[0-9]*.joblib"))
    if not candidates:
        raise FileNotFoundError(f"Không tìm thấy random_forest_*.joblib trong {RESULTS_DIR}")
    model_path   = candidates[-1]
    run_id       = model_path.stem.replace("random_forest_", "")
    imputer_path = RESULTS_DIR / f"random_forest_imputer_{run_id}.joblib"
    if not imputer_path.exists():
        raise FileNotFoundError(f"Không tìm thấy imputer tương ứng: {imputer_path}")
    return model_path, imputer_path


model_path, imputer_path = _find_latest_model()
...
def predict_split(date_filters: list) -> tuple[np.ndarray, np.ndarray]:
    """Load split từ parquet, áp imputer, trả về (y_true, y_prob)."""
...
ax.plot(rec_curve, prec_curve, color="#27AE60", lw=2,
        label=f"Random Forest (AUC-PR={metrics['auc_pr']:.4f})")
...
ax.set_title(f"PR Curve — Random Forest (Test set)\n{model_path.name}")
ax.legend(loc="upper right", fontsize=8)
ax.set_xlim(0, 1)
ax.set_ylim(0, max(prec_curve.max() * 1.1, P_MIN * 2))
plt.tight_layout()

out_fig = RESULTS_DIR / f"rf_pr_curve_{model_path.stem.replace('random_forest_', '')}.png"
...
run_id  = model_path.stem.replace("random_forest_", "")
out_txt = RESULTS_DIR / f"rf_full_metrics_{run_id}.txt"
lines = [
